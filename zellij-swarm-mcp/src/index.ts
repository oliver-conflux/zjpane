...
function validatePaneExists(paneName: string): { valid: true; panes: zjpane.Pane[] } | { valid: false; errorResponse: object } {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case 'spawn_pane': {
        const { name: paneName, direction } = args as {
          name: string;
      ...
      case 'write_to_pane': {
        const { name: paneName, text, send_enter: doSendEnter = true } = args as {
          name: string;
          text: string;
          send_enter?: boolean;
        };
      ...
      case 'send_enter': {
        const { name: paneName } = args as { name: string };
      ...
      case 'read_pane': {
        const { name: paneName, full = false } = args as { name: string; full?: boolean };
      ...
      case 'spawn_agent': {
        const { name: agentName, task, direction, working_dir } = args as {
          name: string;
          task: string;
          direction?: 'up' | 'down' | 'left' | 'right';
          working_dir?: string;
