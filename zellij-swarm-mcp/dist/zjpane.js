 ...
 */
export function validatePaneNotExists(name) {
    const panes = listPanes();
    if (panes.some(p => p.title === name)) {
        throw new ZjpaneError(`Pane "${name}" already exists`, `spawn::${name}`);
    }
}
/**
 ...
 */
export async function waitForPaneCreated(name) {
    ...
    throw new ZjpaneError(`Pane "${name}" failed to spawn within ${SPAWN_TIMEOUT_MS}ms`, `spawn::${name}`);
}
/**
 * Execute a zjpane command via zellij pipe
 */
export function zjpaneCommand(cmd, timeout = 10000) {
    try {
        ...
        if (err.message?.includes('ETIMEDOUT') || err.message?.includes('timed out')) {
            throw new ZjpaneError('zjpane command timed out - is the zjpane plugin loaded? Run: zellij action launch-or-focus-plugin "file:~/.config/zellij/plugins/zjpane.wasm" --floating', cmd);
        }
        throw new ZjpaneError(`zjpane command failed: ${err.message || 'Unknown error'}`, cmd);
    }
}
/**
 ...
 */
export async function spawnPane(name, direction) {
 ...
 */
export function readPane(name, full = false) {
 ...
 */
export function readPaneById(id, full = false) {
 ...
 */
export function closePane(name) {
    zjpaneCommand(`close::${name}`);
}
/**
 * Close a pane by ID
 */
export function closePaneById(id) {
    zjpaneCommand(`close_id::${id}`);
}
/**
 * Focus a pane by name
 */
export function focusPane(name) {
 ...
 */
export function isZjpaneLoaded() {
    try {
        listPanes();
        return true;
    }
